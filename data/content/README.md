...
| `app/knowledge.ts` | 知识点列表（标题 + 摘要 + 时长） | 主包 | 255 KB |
| `app/word-ids.ts` | 单词 id 清单（复习队列选取用） | 主包 | 84 KB |
| `pages-knowledge/content.ts` | 公式 · 说明 · 要点 · 例题 | 分包 | 403 KB |
...
| 项 | 数量 |
| --- | --- |
| 课程 | 4（考研英语、考研数学、考研政治、计算机专业课） |
| 学科 | 8（高等数学、线性代数、概率论、思想政治理论 + 计算机专业课 4） |
| 章节 | 35（全部是数学；政治改走刷题后没有知识点章节） |
| 小节 | 240 |
| 知识点 | 960 |
| 单词 | 5493 |
| 政治题库 | 462 题（写在 `data/politics/questions.ts`，主包只拿套卷投影） |

> 数学来自 `data/math/tree.ts` + `data/math/lectures.ts`；政治来自
> `data/politics/questions.ts`（+ 生成物 `questions-x8.ts`），讲解稿在 `chapters.ts` 里**当前为空**
> ——政治是刷题学科，没有可发布的知识点讲解，所以 `knowledge/` 下没有政治章节。
> 计算机专业课只有学科骨架，还没有章节。
